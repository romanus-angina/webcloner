...
from app.main import app  # Use your actual app
from app.dependencies import get_browser_manager, validate_session_id, get_logger, get_app_state, check_rate_limit

# Create a test app that properly handles dependencies
def create_test_app():
    return FastAPI()

_test_app = create_test_app()
_test_app.include_router(router, prefix="/api/v1/dom")
...
class TestDOMExtractionRoutes:
    ...
    def client(self, mock_dependencies):
        ...
        def override_get_browser_manager():
            return mock_dependencies["browser_manager"]
        
        def override_validate_session_id(session_id: str):
            return session_id  # Just return the session_id without validation
        
        def override_get_logger():
        ...
        _test_app.dependency_overrides[get_browser_manager] = override_get_browser_manager
        _test_app.dependency_overrides[validate_session_id] = override_validate_session_id  
        _test_app.dependency_overrides[get_logger] = override_get_logger
        _test_app.dependency_overrides[get_app_state] = override_get_app_state
        _test_app.dependency_overrides[check_rate_limit] = override_check_rate_limit
    ...
    def test_extract_dom_structure_success(self, client, mock_dependencies):
        ...
        with patch('app.api.routes.dom_extraction.get_app_state', return_value=mock_dependencies["app_state"]), \
             patch('app.api.routes.dom_extraction.get_browser_manager', return_value=mock_dependencies["browser_manager"]), \
             patch('app.api.routes.dom_extraction.get_logger', return_value=mock_dependencies["logger"]), \
             patch('app.api.routes.dom_extraction.get_request_id', return_value="test-request-id"), \
             patch('app.api.routes.dom_extraction.check_rate_limit', return_value=None), \
             patch('app.api.routes.dom_extraction.DOMExtractionService') as mock_service_class:
    ...
    def test_cleanup_session_extractions(self, client, mock_dependencies):
        ...
        with patch('app.api.routes.dom_extraction.validate_session_id', return_value=lambda session_id: session_id), \
             patch('app.api.routes.dom_extraction.get_browser_manager', return_value=mock_dependencies["browser_manager"]), \
             patch('app.api.routes.dom_extraction.get_logger', return_value=mock_dependencies["logger"]), \
             patch('app.api.routes.dom_extraction.DOMExtractionService') as mock_service_class:
            mock_service = AsyncMock()
    ...
    def test_regenerate_extraction_success(self, client, mock_dependencies):
        ...
        with patch('app.api.routes.dom_extraction.DOMExtractionService') as mock_service_class:
            ...
            request_data = {
                "url": "https://example.com",
                "session_id": "test-session",  # Add this line!
                "wait_for_load": True,
                "include_computed_styles": True,
                "max_depth": 10,
                "save_result": True,
    ...
    def test_regenerate_extraction_no_session(self, client, mock_dependencies):
        ...
        with patch('app.api.routes.dom_extraction.validate_session_id', return_value=lambda session_id: session_id), \
             patch('app.api.routes.dom_extraction.get_app_state', return_value=mock_dependencies["app_state"]), \
             patch('app.api.routes.dom_extraction.get_browser_manager', return_value=mock_dependencies["browser_manager"]), \
             patch('app.api.routes.dom_extraction.get_logger', return_value=mock_dependencies["logger"]), \
             patch('app.api.routes.dom_extraction.check_rate_limit', return_value=None):
            request_data = {
    ...
    def test_request_validation(self, client):
        ...
        with patch('app.api.routes.dom_extraction.get_app_state', return_value=MagicMock()), \
             patch('app.api.routes.dom_extraction.get_browser_manager', return_value=AsyncMock()), \
             patch('app.api.routes.dom_extraction.get_logger', return_value=MagicMock()), \
             patch('app.api.routes.dom_extraction.get_request_id', return_value="test-request-id"), \
             patch('app.api.routes.dom_extraction.check_rate_limit', return_value=None), \
             patch('app.api.routes.dom_extraction.DOMExtractionService') as mock_service_class:
            mock_service = AsyncMock()
